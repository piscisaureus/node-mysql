...
function Parser() {
  ...
  // Linked list starting at queueStart
  this.queueStart = null;
  // Insert new queue elems here
  this.queueInsertionPoint = null;
...
Parser.prototype._fetchBytes = function(bytes, callback) {
  if (this.queueInsertAfter) {
    this.queueInsertAfter = this.queueInsertAfter.next = {
      callback: callback,
      bytes: bytes,
      next: this.queueInsertAfter.next
    };
  } else {
    this.queueInsertAfter = this.queueStart = {
      callback: callback,
      bytes: bytes,
...
Parser.prototype._enqueue = function(callback) {
  if (this.queueInsertAfter) {
    ...
    this.queueStart = this.queueInsertAfter = {
      callback: callback,
      bytes: undefined,
      next: this.queueStart
    };
  ...
  this._fetchLCIntStatus(function(status) {
    switch (status) {
      ...
      default:
        ...
        this._emitPacket();
        var remaining = packet.length - packet.received;
        this._fetchBytes(remaining, function(buffer) {
          var lcLength, lcValue;
          var packet = this.packet;
          ...
          for (var i = 0; i < remaining; ) {
            ...
            if (lcValue === null) {
              packet.emit('data', null, 0);
            } else {
              packet.emit('data', buffer.slice(i, i += lcValue), 0);
            }
          }
...
Parser.prototype._call = function() {
  ...
  while (queueStart = this.queueStart) {
    var bytes = queueStart.bytes;

    // If the next callback isn't interested in receiving bytes at all,
    // don't bother to look at the buffer buffer at all
    if (bytes === undefined) {
      this.queueStart = queueStart.next;
      this.queueInsertAfter = null;
      queueStart.callback.call(this);

    } else if (bytes === 0) {
        this.queueStart = queueStart.next;
        this.queueInsertAfter = null;
        queueStart.callback.call(this, new Buffer(0));
      ...
      this.queueStart = queueStart.next;
      this.queueInsertAfter = null;
      queueStart.callback && queueStart.callback.call(this, result);
    }
  }
