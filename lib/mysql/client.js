...
Client.prototype.query = function(sql, params, cb) {
  ...
  var throttleConnection = function() {
    var paused = self._connectionPaused = query._paused;
    if (paused) {
  ...
  if (cb) {
    ...
    query
      .on('error', function(err) {
        query.removeListener('throttle', throttleConnection);
        cb(err);
        self._dequeue();
      })
  ...
  this._enqueue(function() {
    ...
    if (query._paused != self._connectionPaused) {
      throttleConnection();
    }
    query.on('throttle', throttleConnection);
  }, query);
