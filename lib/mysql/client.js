...
Client.prototype.query = function(sql, params, cb) {
  ...
  var throttlePackets = function() {
    var paused = self._paused = query._paused;
    if (paused) {
  ...
  if (cb) {
    ...
    query
      .on('error', function(err) {
        query.removeListener('throttle', throttlePackets);
        cb(err);
        self._dequeue();
      })
  ...
  this._enqueue(function query() {
    ...
    if (query._paused != self._paused) {
      throttlePackets();
    }
    query.on('throttle', throttlePackets);
  }, query);
